package ddb.smartcenter.gateway.listener;
...
public class DDBActivtyListener implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent even) {
    	...
    	if(ActConfig.getGlobalDebug()){
    		...
    		JDBC.setDefultDataSourceProvider(DDBActivtyDataSourceProviderImpl.getInstance(), "java:comp/env/jdbc/AppGateWay");
    	}
    	//加载pubApps
	...
	public void loadConfig(){
		//1设置项目名
	    ConfigurePool.setProjectName("AIGateWay");
	    //2取标准配置文件对象,这个引用外部系统可以持有
	    IConfigure ic = ConfigurePool.getStdConfig();
	    //3读取配置的键值对值
	   ddb.smartcenter.gateway.util.ConfigUtil.configure=ic;
